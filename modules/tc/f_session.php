...
class TcSessionHelper {
    ...
    const MAX_USERS = 80;
    const MAX_USERS_RECOMMENDATION_RATIO = 2; //recommends USERS/RATIO to user
    
    
    public function __construct($type,$cid,$ccod) {
        $this->tc_type = $type;
        $this->course_id = $cid;
        $this->course_code = $ccod;
    }
    ...
    public function form($session_id=0) {
        ...
        $server = (new TcServer())->LoadByType($this->tc_type,true);
        if ( !$server )
            die('[TcSessionHelper] No servers enabled');
        
        $tool_content = "
        <div class='form-wrapper'>
        <form class='form-horizontal' role='form' name='sessionForm' action='$_SERVER[SCRIPT_NAME]?course=$this->course_code' method='post' >
        <fieldset>
     ...
     * @param string $title
     * @param string $desc
     * @param string $start_session -- startDate in DB
     * @param string $BBBEndDate
     * @param int $status -- 1/0 (active)
     * @param string $notifyUsers -- "1" / "0" -- booleans are not good enough apparently
     * @param string $notifyExternalUsers -- "1" / "0"
     * @param string $addAnnouncement -- "1" / "0"
     * @param int  $minutes_before -- unlock_interval in DB
     * @param string $external_users
     * @param string $record --"true" / "false" -- gods help us
     * @param int $sessionUsers
     * @param array $groups - array of: 0=all users, _X=group id X, X=user X
     * @param int $session_id
     * @return bool
     */
    function add_update($title, $desc, $start_session, $BBBEndDate, $status, $notifyUsers, $notifyExternalUsers, $addAnnouncement,
        $minutes_before, $external_users, $record, $sessionUsers, $groups, $session_id = 0)
    {
        ...
        if ( $session_id != 0 ) { //updating/editing session
            ...
            if ($t) {
                $server_id = Database::get()->querySingle("SELECT id FROM tc_servers WHERE id = ?d AND `type` = ?s AND enabled = 'true' ORDER BY weight ASC",
                    $t->external_server,$tc_type)->id;
            } else { // else course will use default tc_server
                $server_id = Database::get()->querySingle("SELECT id FROM tc_servers WHERE `type` = ?s and enabled = 'true' ORDER BY weight ASC",
                    $tc_type)->id;
            }
        ...
        if ($notifyUsers == "1" && is_array($groups) and count($groups) > 0) {
            ...
            if (count($recipients) > 0) {
                ...
                $bbblink = $urlServer . "modules/tc/index.php?course=$course_code&amp;choice=do_join&amp;meeting_id=$new_meeting_id&amp;title=" .
                urlencode($new_title) . "&amp;att_pw=$new_att_pw";
                $emailheader = "
                <div id='mail-header'>
                    <div>
     ...
     * @return bool
     */
    function disable($session_id)
    {
        $x = Database::get()->querySingle("UPDATE tc_session set active='0' WHERE id=?d",$session_id);
        return $x!==NULL;
    }
     ...
     * @return bool
     */
    function enable($session_id)
    {
        $x = Database::get()->querySingle("UPDATE tc_session SET active='1' WHERE id=?d",$session_id);
        return $x!==NULL;
    }
    ...
    {
        $q = Database::get()->querySingle("SELECT title FROM tc_session WHERE id = ?d", $session_id);
        if (!$q)
            return false;
        $tc_title = $q->title;
        $q = Database::get()->querySingle("DELETE FROM tc_session WHERE id = ?d", $session_id);
        if ( !$q )
            return false;
        Log::record($this->course_id, MODULE_ID_TC, LOG_DELETE, array('id' => $session_id,'title' => $tc_title));
        return true;
    }
     ...
     * @return boolean
     */
    function IsMeetingRunning($meeting_id)
    {
        ...
        try {
            $result = $bbb->isMeetingRunning($meeting_id);
        }
        catch (Exception $e) {
            return false;
        }
 ...
 */
function tc_session_details($tc_type,$course_id,$course_code) {

    global $course_id, $tool_content, $is_editor, $uid,
    ...
    $isActiveTcServer = is_active_tc_server($tc_type, $course_id); //cache this since it involves DB queries
    ...
    $result = Database::get()->queryArray("SELECT * FROM tc_session WHERE course_id = ?d $activeClause
    ...
    if ($result) {
        ...
        foreach ($result as $row) {
            ...
            elseif (($row->active == '1') and (date_diff_in_minutes($start_date, date('Y-m-d H:i:s')) < $row->unlock_interval)
                and $isActiveTcServer) {
                $canJoin = TRUE;
            }
            else
                $canJoin = FALSE;
