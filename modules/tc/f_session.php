...
class TcSessionHelper {
    ...
    const AVAILABLE_APIS = [ 'bbb'=>'BigBlueButton', 'om'=>'OpenMeetings', 'webconf'=>'WebConf' ];
    ...
    public function getSessionByIdAndType($id) {
    ...
    public function form($session_id=0) {
        ...
        $server = TcServer::LoadOneByType($this->tc_type,true);
        if ( !$server )
                        ...
                        <td class='option-btn-cell'>".
                            action_button(array(
                                array(  'title' => $langEditChange,
                                        'url' => "$_SERVER[SCRIPT_NAME]?course=$this->course_code&amp;id=" . getIndirectReference($id) . "&amp;choice=edit",
                                        'icon' => 'fa-edit'),
                                array(  'title' => $langBBBImportRecordings,
                                        'url' => "$_SERVER[SCRIPT_NAME]?course=$this->course_code&amp;id=" . getIndirectReference($row->id) . "&amp;choice=import_video",
                                        'icon' => "fa-edit",
                                        'show' => $this->tc_type == 'bbb'),
                                array(  'title' => $langParticipate,
                                        'url' => "tcuserduration.php?course=$this->course_code&amp;id=$row->id",
                                        'icon' => "fa-clock-o",
                                        'show' => $this->tc_type == 'bbb'),
                                array(  'title' => $row->active? $langDeactivate : $langActivate,
                                        'url' => "$_SERVER[SCRIPT_NAME]?course=$this->course_code&amp;id=" . getIndirectReference($row->id) . "&amp;choice=do_".
                                                 ($row->active? 'disable' : 'enable'),
                                        'icon' => $row->active ? 'fa-eye': 'fa-eye-slash'),
                                array(  'title' => $langDelete,
                                        'url' => "$_SERVER[SCRIPT_NAME]?course=$this->course_code&amp;id=" . getIndirectReference($row->id) . "&amp;choice=do_delete",
                                        'icon' => 'fa-times',
                                        'class' => 'delete',
    ...
    function is_active_tc_server() {
        ...
        $q = Database::get()->queryArray("SELECT id, all_courses FROM tc_servers WHERE enabled='true'
                                AND `type` = ?s ORDER BY weight",$this->tc_type);
