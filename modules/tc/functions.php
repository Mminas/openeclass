...
function publish_video_recordings($course_id, $id)
{
    ...
    $sessions = Database::get()->queryArray("SELECT tc_session.id, tc_session.course_id AS course_id," . "tc_session.title, tc_session.description, tc_session.start_date," . "tc_session.meeting_id, course.prof_names FROM tc_session " . "LEFT JOIN course ON tc_session.course_id=course.id WHERE course.code=?s AND tc_session.id=?d", $course_id, $id);
...
function is_configured_tc_server() {
    global $course_id;
    //$tch = new TcSessionHelper($course_id,''); //course_code not needed
    //return count($tch::$tc_types)>0;
